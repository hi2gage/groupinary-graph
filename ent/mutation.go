...
const (
	...
	TypeGroup           = "Group"
	TypeWord            = "Word"
	TypeWordConnections = "WordConnections"
)
...
// WordConnectionsMutation represents an operation that mutates the WordConnections nodes in the graph.
type WordConnectionsMutation struct {
	config
	op            Op
	typ           string
	id            *int
	...
	oldValue      func(context.Context) (*WordConnections, error)
	predicates    []predicate.WordConnections
}

var _ ent.Mutation = (*WordConnectionsMutation)(nil)

// wordconnectionsOption allows management of the mutation configuration using functional options.
type wordconnectionsOption func(*WordConnectionsMutation)

// newWordConnectionsMutation creates new mutation for the WordConnections entity.
func newWordConnectionsMutation(c config, op Op, opts ...wordconnectionsOption) *WordConnectionsMutation {
	m := &WordConnectionsMutation{
		config:        c,
		op:            op,
		typ:           TypeWordConnections,
		clearedFields: make(map[string]struct{}),
	}
	for _, opt := range opts {
		opt(m)
...
// withWordConnectionsID sets the ID field of the mutation.
func withWordConnectionsID(id int) wordconnectionsOption {
	return func(m *WordConnectionsMutation) {
		var (
			err   error
			once  sync.Once
			value *WordConnections
		)
		m.oldValue = func(ctx context.Context) (*WordConnections, error) {
			once.Do(func() {
				if m.done {
					err = errors.New("querying old values post mutation is not allowed")
				} else {
					value, err = m.Client().WordConnections.Get(ctx, id)
				}
			})
			return value, err
		}
...
// withWordConnections sets the old WordConnections of the mutation.
func withWordConnections(node *WordConnections) wordconnectionsOption {
	return func(m *WordConnectionsMutation) {
		m.oldValue = func(context.Context) (*WordConnections, error) {
			return node, nil
		}
		m.id = &node.ID
	}
...
// Client returns a new `ent.Client` from the mutation. If the mutation was
// executed in a transaction (ent.Tx), a transactional client is returned.
func (m WordConnectionsMutation) Client() *Client {
	client := &Client{config: m.config}
	client.init()
	return client
}
...
// Tx returns an `ent.Tx` for mutations that were executed in transactions;
// it returns an error otherwise.
func (m WordConnectionsMutation) Tx() (*Tx, error) {
	if _, ok := m.driver.(*txDriver); !ok {
		return nil, errors.New("ent: mutation is not running in a transaction")
	}
	tx := &Tx{config: m.config}
...
// ID returns the ID value in the mutation. Note that the ID is only available
// if it was provided to the builder or after it was returned from the database.
func (m *WordConnectionsMutation) ID() (id int, exists bool) {
	if m.id == nil {
		return
	}
	return *m.id, true
...
// as sql.LevelSerializable, the returned ids match the ids of the rows that will be updated
// or updated by the mutation.
func (m *WordConnectionsMutation) IDs(ctx context.Context) ([]int, error) {
	switch {
	case m.op.Is(OpUpdateOne | OpDeleteOne):
		id, exists := m.ID()
		if exists {
		...
		fallthrough
	case m.op.Is(OpUpdate | OpDelete):
		return m.Client().WordConnections.Query().Where(m.predicates...).IDs(ctx)
	default:
		return nil, fmt.Errorf("IDs is not allowed on %s operations", m.op)
	}
}
...
// SetDescription sets the "description" field.
func (m *WordConnectionsMutation) SetDescription(s string) {
	m.description = &s
}

// Description returns the value of the "description" field in the mutation.
func (m *WordConnectionsMutation) Description() (r string, exists bool) {
	v := m.description
	if v == nil {
		return
	}
...
// OldDescription returns the old "description" field's value of the WordConnections entity.
// If the WordConnections object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *WordConnectionsMutation) OldDescription(ctx context.Context) (v string, err error) {
	if !m.op.Is(OpUpdateOne) {
		return v, errors.New("OldDescription is only allowed on UpdateOne operations")
	}
	if m.id == nil || m.oldValue == nil {
...
// ResetDescription resets all changes to the "description" field.
func (m *WordConnectionsMutation) ResetDescription() {
	m.description = nil
}

// Where appends a list predicates to the WordConnectionsMutation builder.
func (m *WordConnectionsMutation) Where(ps ...predicate.WordConnections) {
	m.predicates = append(m.predicates, ps...)
}

// WhereP appends storage-level predicates to the WordConnectionsMutation builder. Using this method,
// users can use type-assertion to append predicates that do not depend on any generated package.
func (m *WordConnectionsMutation) WhereP(ps ...func(*sql.Selector)) {
	p := make([]predicate.WordConnections, len(ps))
	for i := range ps {
		p[i] = ps[i]
	}
	m.Where(p...)
...
// Op returns the operation name.
func (m *WordConnectionsMutation) Op() Op {
	return m.op
}

// SetOp allows setting the mutation operation.
func (m *WordConnectionsMutation) SetOp(op Op) {
	m.op = op
}

// Type returns the node type of this mutation (WordConnections).
func (m *WordConnectionsMutation) Type() string {
	return m.typ
}
...
// order to get all numeric fields that were incremented/decremented, call
// AddedFields().
func (m *WordConnectionsMutation) Fields() []string {
	fields := make([]string, 0, 1)
	if m.description != nil {
		fields = append(fields, wordconnections.FieldDescription)
	}
	return fields
}
...
// return value indicates that this field was not set, or was not defined in the
// schema.
func (m *WordConnectionsMutation) Field(name string) (ent.Value, bool) {
	switch name {
...
// returned if the mutation operation is not UpdateOne, or the query to the
// database failed.
func (m *WordConnectionsMutation) OldField(ctx context.Context, name string) (ent.Value, error) {
	switch name {
	case wordconnections.FieldDescription:
		return m.OldDescription(ctx)
	}
	return nil, fmt.Errorf("unknown WordConnections field %s", name)
}

// SetField sets the value of a field with the given name. It returns an error if
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *WordConnectionsMutation) SetField(name string, value ent.Value) error {
	switch name {
	case wordconnections.FieldDescription:
		v, ok := value.(string)
		if !ok {
	...
	return fmt.Errorf("unknown WordConnections field %s", name)
}

// AddedFields returns all numeric fields that were incremented/decremented during
// this mutation.
func (m *WordConnectionsMutation) AddedFields() []string {
	return nil
}
...
// with the given name. The second boolean return value indicates that this field
// was not set, or was not defined in the schema.
func (m *WordConnectionsMutation) AddedField(name string) (ent.Value, bool) {
	return nil, false
}
...
// the field is not defined in the schema, or if the type mismatched the field
// type.
func (m *WordConnectionsMutation) AddField(name string, value ent.Value) error {
	switch name {
	}
	return fmt.Errorf("unknown WordConnections numeric field %s", name)
}

// ClearedFields returns all nullable fields that were cleared during this
// mutation.
func (m *WordConnectionsMutation) ClearedFields() []string {
	return nil
}

// FieldCleared returns a boolean indicating if a field with the given name was
// cleared in this mutation.
func (m *WordConnectionsMutation) FieldCleared(name string) bool {
	_, ok := m.clearedFields[name]
	return ok
}
...
// ClearField clears the value of the field with the given name. It returns an
// error if the field is not defined in the schema.
func (m *WordConnectionsMutation) ClearField(name string) error {
	return fmt.Errorf("unknown WordConnections nullable field %s", name)
}

// ResetField resets all changes in the mutation for the field with the given name.
// It returns an error if the field is not defined in the schema.
func (m *WordConnectionsMutation) ResetField(name string) error {
	switch name {
	case wordconnections.FieldDescription:
		m.ResetDescription()
		return nil
...
// AddedEdges returns all edge names that were set/added in this mutation.
func (m *WordConnectionsMutation) AddedEdges() []string {
	edges := make([]string, 0, 0)
	return edges
}
...
// AddedIDs returns all IDs (to other nodes) that were added for the given edge
// name in this mutation.
func (m *WordConnectionsMutation) AddedIDs(name string) []ent.Value {
	return nil
}

// RemovedEdges returns all edge names that were removed in this mutation.
func (m *WordConnectionsMutation) RemovedEdges() []string {
	edges := make([]string, 0, 0)
	return edges
}
...
// RemovedIDs returns all IDs (to other nodes) that were removed for the edge with
// the given name in this mutation.
func (m *WordConnectionsMutation) RemovedIDs(name string) []ent.Value {
	return nil
}

// ClearedEdges returns all edge names that were cleared in this mutation.
func (m *WordConnectionsMutation) ClearedEdges() []string {
	edges := make([]string, 0, 0)
	return edges
}
...
// EdgeCleared returns a boolean which indicates if the edge with the given name
// was cleared in this mutation.
func (m *WordConnectionsMutation) EdgeCleared(name string) bool {
	return false
}

// ClearEdge clears the value of the edge with the given name. It returns an error
// if that edge is not defined in the schema.
func (m *WordConnectionsMutation) ClearEdge(name string) error {
	return fmt.Errorf("unknown WordConnections unique edge %s", name)
}

// ResetEdge resets all changes to the edge with the given name in this mutation.
// It returns an error if the edge is not defined in the schema.
func (m *WordConnectionsMutation) ResetEdge(name string) error {
	return fmt.Errorf("unknown WordConnections edge %s", name)
}
